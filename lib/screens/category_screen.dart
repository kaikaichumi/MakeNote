  ...
  Widget _buildCategoryTree() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return _buildCategoryItem(topLevelCategories[index], 0);
      },
    );
  }

  // 構建類別項目
  Widget _buildCategoryItem(Category category, int depth) {
    // 查找子類別
    final children = _categories.where((cat) => cat.parentId == category.id).toList();
